// ==================== TLC Calisthenics — Track Definitions ====================
import type { Track } from './types';

export const tracks: Track[] = [
  {
    id: 'planche',
    name: 'Planche',
    description: 'From push-up to planche. Straight-arm strength, protraction, and leverage mastery.',
    icon: 'Zap',
    nodes: [
      { exerciseId: 'planche-lean', prereqs: [], unlockTest: { type: 'hold', value: 20, unit: 'sec' } },
      { exerciseId: 'pseudo-planche-push-up', prereqs: ['planche-lean'], unlockTest: { type: 'reps', value: 8, unit: 'reps' } },
      { exerciseId: 'planche-plank', prereqs: ['planche-lean'], unlockTest: { type: 'hold', value: 20, unit: 'sec' } },
      { exerciseId: 'tuck-planche', prereqs: ['planche-plank', 'pseudo-planche-push-up'], unlockTest: { type: 'hold', value: 10, unit: 'sec' } },
      { exerciseId: 'advanced-tuck-planche', prereqs: ['tuck-planche'], unlockTest: { type: 'hold', value: 10, unit: 'sec' } },
      { exerciseId: 'straddle-planche', prereqs: ['advanced-tuck-planche'], unlockTest: { type: 'hold', value: 5, unit: 'sec' } },
      { exerciseId: 'full-planche', prereqs: ['straddle-planche'] },
    ],
  },
  {
    id: 'handstand',
    name: 'Handstand',
    description: 'Frog stand through elbow lever to freestanding handstand. Build the line, then own the balance.',
    icon: 'ArrowUp',
    nodes: [
      { exerciseId: 'frog-stand', prereqs: [], unlockTest: { type: 'hold', value: 15, unit: 'sec' } },
      { exerciseId: 'elbow-lever', prereqs: ['frog-stand'], unlockTest: { type: 'hold', value: 10, unit: 'sec' } },
      { exerciseId: 'single-arm-elbow-lever', prereqs: ['elbow-lever'], unlockTest: { type: 'hold', value: 5, unit: 'sec' } },
      { exerciseId: 'crocodile-pose', prereqs: ['elbow-lever'], unlockTest: { type: 'hold', value: 10, unit: 'sec' } },
      { exerciseId: 'hollow-body-hold', prereqs: ['elbow-lever'], unlockTest: { type: 'hold', value: 30, unit: 'sec' } },
      { exerciseId: 'chest-to-wall-handstand', prereqs: ['hollow-body-hold'], unlockTest: { type: 'hold', value: 45, unit: 'sec' } },
      { exerciseId: 'straddle-hs-kick-up', prereqs: ['chest-to-wall-handstand'], unlockTest: { type: 'reps', value: 5, unit: 'reps' } },
      { exerciseId: 'hs-shrugs', prereqs: ['chest-to-wall-handstand'], unlockTest: { type: 'reps', value: 10, unit: 'reps' } },
      { exerciseId: 'wall-pull-aways', prereqs: ['hs-shrugs'], unlockTest: { type: 'hold', value: 5, unit: 'sec' } },
      { exerciseId: 'handstand-bail', prereqs: ['wall-pull-aways'], unlockTest: { type: 'reps', value: 3, unit: 'reps' } },
      { exerciseId: 'freestanding-handstand', prereqs: ['wall-pull-aways', 'handstand-bail'] },
      { exerciseId: 'bent-arm-handstand', prereqs: ['freestanding-handstand'] },
    ],
  },
  {
    id: 'rings',
    name: 'Rings',
    description: 'Support hold to muscle-up pipeline. Control instability.',
    icon: 'Circle',
    nodes: [
      { exerciseId: 'ring-support-hold', prereqs: [], unlockTest: { type: 'hold', value: 20, unit: 'sec' } },
      { exerciseId: 'rto-support-hold', prereqs: ['ring-support-hold'], unlockTest: { type: 'hold', value: 15, unit: 'sec' } },
      { exerciseId: 'ring-row', prereqs: ['ring-support-hold'], unlockTest: { type: 'reps', value: 10, unit: 'reps' } },
      { exerciseId: 'ring-dip', prereqs: ['rto-support-hold'], unlockTest: { type: 'reps', value: 5, unit: 'reps' } },
      { exerciseId: 'false-grip-hold', prereqs: ['rto-support-hold'], unlockTest: { type: 'hold', value: 15, unit: 'sec' } },
      { exerciseId: 'muscle-up', prereqs: ['ring-dip', 'false-grip-hold'] },
    ],
  },
  {
    id: 'compression',
    name: 'Compression',
    description: 'Hanging raises to press handstand. Active hip flexion, pike and straddle compression mastery.',
    icon: 'Minimize2',
    nodes: [
      { exerciseId: 'hollow-body-hold', prereqs: [], unlockTest: { type: 'hold', value: 30, unit: 'sec' } },
      { exerciseId: 'plank', prereqs: [], unlockTest: { type: 'hold', value: 60, unit: 'sec' } },
      { exerciseId: 'hanging-leg-raise', prereqs: ['hollow-body-hold'], unlockTest: { type: 'reps', value: 8, unit: 'reps' } },
      { exerciseId: 'tuck-sit', prereqs: ['hollow-body-hold'], unlockTest: { type: 'hold', value: 15, unit: 'sec' } },
      { exerciseId: 'seated-pike-lift', prereqs: ['hollow-body-hold'], unlockTest: { type: 'reps', value: 8, unit: 'reps' } },
      { exerciseId: 'seated-straddle-lift', prereqs: ['tuck-sit'], unlockTest: { type: 'reps', value: 8, unit: 'reps' } },
      { exerciseId: 'l-sit', prereqs: ['tuck-sit'], unlockTest: { type: 'hold', value: 10, unit: 'sec' } },
      { exerciseId: 'wall-compression-stretch', prereqs: ['seated-pike-lift'], unlockTest: { type: 'hold', value: 30, unit: 'sec' } },
      { exerciseId: 'elevated-chair-compression', prereqs: ['wall-compression-stretch', 'l-sit'], unlockTest: { type: 'hold', value: 20, unit: 'sec' } },
      { exerciseId: 'straddle-press-to-hs', prereqs: ['elevated-chair-compression'], unlockTest: { type: 'reps', value: 3, unit: 'reps' } },
      { exerciseId: 'stalder-press-to-hs', prereqs: ['straddle-press-to-hs'] },
    ],
  },
  {
    id: 'pull-strength',
    name: 'Pull Strength',
    description: 'Australian row to front lever. Build your back.',
    icon: 'ArrowDown',
    nodes: [
      { exerciseId: 'australian-row', prereqs: [], unlockTest: { type: 'reps', value: 12, unit: 'reps' } },
      { exerciseId: 'chin-up', prereqs: ['australian-row'], unlockTest: { type: 'reps', value: 8, unit: 'reps' } },
      { exerciseId: 'pull-up', prereqs: ['chin-up'], unlockTest: { type: 'reps', value: 8, unit: 'reps' } },
      { exerciseId: 'skin-the-cat', prereqs: ['pull-up'], unlockTest: { type: 'reps', value: 5, unit: 'reps' } },
      { exerciseId: 'pullover', prereqs: ['pull-up'], unlockTest: { type: 'reps', value: 3, unit: 'reps' } },
      { exerciseId: 'archer-pull-up', prereqs: ['pull-up'], unlockTest: { type: 'reps', value: 5, unit: 'reps' } },
      { exerciseId: 'back-lever', prereqs: ['skin-the-cat'], unlockTest: { type: 'hold', value: 10, unit: 'sec' } },
      { exerciseId: 'front-lever-raise', prereqs: ['pull-up'], unlockTest: { type: 'reps', value: 3, unit: 'reps' } },
      { exerciseId: 'front-lever', prereqs: ['front-lever-raise'] },
    ],
  },
  {
    id: 'push-strength',
    name: 'Push Strength',
    description: 'Push-up to HSPU. The complete pressing pipeline.',
    icon: 'TrendingUp',
    nodes: [
      { exerciseId: 'push-up', prereqs: [], unlockTest: { type: 'reps', value: 20, unit: 'reps' } },
      { exerciseId: 'diamond-push-up', prereqs: ['push-up'], unlockTest: { type: 'reps', value: 12, unit: 'reps' } },
      { exerciseId: 'archer-push-up', prereqs: ['push-up'], unlockTest: { type: 'reps', value: 8, unit: 'reps' } },
      { exerciseId: 'parallel-bar-dip', prereqs: ['push-up'], unlockTest: { type: 'reps', value: 10, unit: 'reps' } },
      { exerciseId: 'pike-push-up', prereqs: ['diamond-push-up'], unlockTest: { type: 'reps', value: 10, unit: 'reps' } },
      { exerciseId: 'tiger-bend-pushup', prereqs: ['pike-push-up'], unlockTest: { type: 'reps', value: 5, unit: 'reps' } },
      { exerciseId: 'hspu-capacity-builder', prereqs: ['pike-push-up'], unlockTest: { type: 'reps', value: 8, unit: 'reps' } },
      { exerciseId: 'handstand-push-up', prereqs: ['hspu-capacity-builder', 'tiger-bend-pushup'] },
    ],
  },
  {
    id: 'legs',
    name: 'Legs',
    description: 'Squat to nordic. Single-leg strength progression.',
    icon: 'Footprints',
    nodes: [
      { exerciseId: 'bodyweight-squat', prereqs: [], unlockTest: { type: 'reps', value: 20, unit: 'reps' } },
      { exerciseId: 'lunge', prereqs: ['bodyweight-squat'], unlockTest: { type: 'reps', value: 12, unit: 'reps' } },
      { exerciseId: 'cossack-squat', prereqs: ['bodyweight-squat'], unlockTest: { type: 'reps', value: 8, unit: 'reps' } },
      { exerciseId: 'bulgarian-split-squat', prereqs: ['bodyweight-squat'], unlockTest: { type: 'reps', value: 10, unit: 'reps' } },
      { exerciseId: 'pistol-squat', prereqs: ['bulgarian-split-squat'], unlockTest: { type: 'reps', value: 5, unit: 'reps' } },
      { exerciseId: 'shrimp-squat', prereqs: ['pistol-squat'], unlockTest: { type: 'reps', value: 3, unit: 'reps' } },
      { exerciseId: 'nordic-curl', prereqs: ['bodyweight-squat'], unlockTest: { type: 'reps', value: 5, unit: 'reps' } },
      { exerciseId: 'calf-raise', prereqs: [], unlockTest: { type: 'reps', value: 20, unit: 'reps' } },
    ],
  },
  {
    id: 'inversions',
    name: 'Inversions',
    description: 'Dolphin to Pincha to Elbow Stand. All forearm-supported inversions in one track.',
    icon: 'Hand',
    nodes: [
      { exerciseId: 'dolphin-pose', prereqs: [], unlockTest: { type: 'hold', value: 30, unit: 'sec' } },
      { exerciseId: 'forearm-stand-toe-pulls', prereqs: ['dolphin-pose'], unlockTest: { type: 'hold', value: 5, unit: 'sec' } },
      { exerciseId: 'forearm-stand-line-hold', prereqs: ['forearm-stand-toe-pulls'], unlockTest: { type: 'hold', value: 10, unit: 'sec' } },
      { exerciseId: 'elbow-stand-wall-hold', prereqs: ['dolphin-pose'], unlockTest: { type: 'hold', value: 20, unit: 'sec' } },
      { exerciseId: 'elbow-stand-straddle-entry', prereqs: ['elbow-stand-wall-hold'], unlockTest: { type: 'reps', value: 3, unit: 'reps' } },
      { exerciseId: 'elbow-stand-exit', prereqs: ['elbow-stand-wall-hold'] },
    ],
  },
  {
    id: 'flag',
    name: 'Flag',
    description: 'Dragon Flag to Human Flag. Lateral and anti-extension mastery.',
    icon: 'Flag',
    nodes: [
      { exerciseId: 'dragon-flag', prereqs: [], unlockTest: { type: 'reps', value: 5, unit: 'reps' } },
      { exerciseId: 'clutch-flag', prereqs: ['dragon-flag'], unlockTest: { type: 'hold', value: 5, unit: 'sec' } },
      { exerciseId: 'human-flag', prereqs: ['clutch-flag'] },
    ],
  },
  {
    id: 'grip',
    name: 'Grip',
    description: 'Dead hang to towel hang. Build forearm endurance and extensor balance.',
    icon: 'Grip',
    nodes: [
      { exerciseId: 'dead-hang', prereqs: [], unlockTest: { type: 'hold', value: 30, unit: 'sec' } },
      { exerciseId: 'towel-hang', prereqs: ['dead-hang'], unlockTest: { type: 'hold', value: 15, unit: 'sec' } },
      { exerciseId: 'extensor-band', prereqs: [] },
    ],
  },
  {
    id: 'press',
    name: 'Press',
    description: 'Wall compression to stalder press. The press-to-handstand pipeline.',
    icon: 'TrendingUp',
    nodes: [
      { exerciseId: 'wall-compression-stretch', prereqs: [], unlockTest: { type: 'hold', value: 30, unit: 'sec' } },
      { exerciseId: 'elevated-chair-compression', prereqs: ['wall-compression-stretch'], unlockTest: { type: 'hold', value: 20, unit: 'sec' } },
      { exerciseId: 'press-hs-journey', prereqs: ['elevated-chair-compression'], unlockTest: { type: 'reps', value: 5, unit: 'reps' } },
      { exerciseId: 'straddle-press-to-hs', prereqs: ['press-hs-journey'], unlockTest: { type: 'reps', value: 3, unit: 'reps' } },
      { exerciseId: 'stalder-press-to-hs', prereqs: ['straddle-press-to-hs'] },
    ],
  },
  {
    id: 'mobility',
    name: 'Mobility & Flexibility',
    description: 'Cat-Cow to Middle Split. Build flexibility, mobility, and total range of motion.',
    icon: 'Stretch',
    nodes: [
      { exerciseId: 'cat-cow-stretch', prereqs: [], unlockTest: { type: 'reps', value: 10, unit: 'reps' } },
      { exerciseId: 'pigeon-pose', prereqs: ['cat-cow-stretch'], unlockTest: { type: 'hold', value: 60, unit: 'sec' } },
      { exerciseId: 'cossack-squat', prereqs: ['cat-cow-stretch'], unlockTest: { type: 'reps', value: 8, unit: 'reps' } },
      { exerciseId: 'pancake-stretch', prereqs: ['pigeon-pose'], unlockTest: { type: 'hold', value: 60, unit: 'sec' } },
      { exerciseId: 'middle-split-hold', prereqs: ['pancake-stretch'], unlockTest: { type: 'hold', value: 30, unit: 'sec' } },
      { exerciseId: 'needle', prereqs: ['pancake-stretch'] },
      { exerciseId: 'wheel-pose', prereqs: ['pigeon-pose'] },
    ],
  },
  {
    id: 'yoga-flow',
    name: 'Yoga Flow',
    description: 'Child\'s Pose to Wheel. Build flexibility, balance, and body awareness through classical yoga progression.',
    icon: 'Flower2',
    nodes: [
      { exerciseId: 'childs-pose', prereqs: [], unlockTest: { type: 'hold', value: 60, unit: 'sec' } },
      { exerciseId: 'cat-cow-stretch', prereqs: ['childs-pose'], unlockTest: { type: 'reps', value: 10, unit: 'reps' } },
      { exerciseId: 'downward-dog', prereqs: ['cat-cow-stretch'], unlockTest: { type: 'hold', value: 45, unit: 'sec' } },
      { exerciseId: 'cobra-pose', prereqs: ['cat-cow-stretch'], unlockTest: { type: 'hold', value: 30, unit: 'sec' } },
      { exerciseId: 'warrior-i', prereqs: ['downward-dog'], unlockTest: { type: 'hold', value: 45, unit: 'sec' } },
      { exerciseId: 'warrior-ii', prereqs: ['warrior-i'], unlockTest: { type: 'hold', value: 45, unit: 'sec' } },
      { exerciseId: 'triangle-pose', prereqs: ['warrior-ii'], unlockTest: { type: 'hold', value: 30, unit: 'sec' } },
      { exerciseId: 'tree-pose', prereqs: ['warrior-i'], unlockTest: { type: 'hold', value: 30, unit: 'sec' } },
      { exerciseId: 'warrior-iii', prereqs: ['warrior-ii', 'tree-pose'], unlockTest: { type: 'hold', value: 15, unit: 'sec' } },
      { exerciseId: 'chair-pose', prereqs: ['warrior-i'], unlockTest: { type: 'hold', value: 30, unit: 'sec' } },
      { exerciseId: 'crow-pose', prereqs: ['chair-pose'], unlockTest: { type: 'hold', value: 10, unit: 'sec' } },
      { exerciseId: 'camel-pose', prereqs: ['cobra-pose'], unlockTest: { type: 'hold', value: 20, unit: 'sec' } },
      { exerciseId: 'half-moon-pose', prereqs: ['triangle-pose', 'warrior-iii'], unlockTest: { type: 'hold', value: 15, unit: 'sec' } },
      { exerciseId: 'pigeon-pose', prereqs: ['downward-dog'], unlockTest: { type: 'hold', value: 60, unit: 'sec' } },
      { exerciseId: 'bridge-pose', prereqs: ['camel-pose'], unlockTest: { type: 'hold', value: 20, unit: 'sec' } },
      { exerciseId: 'dolphin-pose', prereqs: ['downward-dog'], unlockTest: { type: 'hold', value: 30, unit: 'sec' } },
      { exerciseId: 'wheel-pose', prereqs: ['bridge-pose'] },
    ],
  },
];

export const getTrackById = (id: string) => tracks.find(t => t.id === id);
