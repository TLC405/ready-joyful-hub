...
export function CommandSearch({ open, onClose }: CommandSearchProps) {
  ...
  const results = useMemo(() => {
    ...
    return exercises.filter(e => e.name.toLowerCase().includes(query.toLowerCase())).slice(0, 8);
  }, [query]);
  ...
  const handleSelect = (id: string) => { navigate(`/video/${id}`); onClose(); };

  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'ArrowDown') { e.preventDefault(); setSelectedIndex(i => Math.min(i + 1, results.length - 1)); }
    else if (e.key === 'ArrowUp') { e.preventDefault(); setSelectedIndex(i => Math.max(i - 1, 0)); }
    else if (e.key === 'Enter' && results[selectedIndex]) { handleSelect(results[selectedIndex].id); }
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[15vh]" onClick={onClose}>
      ...
      <div
        className="relative z-10 w-full max-w-lg border-2 border-foreground bg-card shadow-2xl skeuo-thunder-card skeuo-grain"
        onClick={e => e.stopPropagation()}
      >
        {/* Search input */}
        <div className="flex items-center gap-3 border-b-2 border-foreground px-4 py-3 thunder-inset">
          <Search className="h-5 w-5 text-thunder-orange shrink-0" />
          <input
            ref={inputRef}
            type="text"
            value={query}
          ...
          />
          <button onClick={onClose} className="text-muted-foreground hover:text-thunder-orange">
            <X className="h-4 w-4" />
          </button>
        </div>
            ...
            results.map((ex, i) => (
              <button
                ...
                className={cn(
                  ...
                  i === selectedIndex ? "thunder-gradient-subtle" : "hover:bg-surface-0"
                )}
              >
                ...
                {(ex.videoUrl || ex.videoSources?.length) ? (
                  <Play className="h-3.5 w-3.5 text-thunder-orange shrink-0" />
                ) : (
        ...
        <div className="flex items-center justify-between border-t border-foreground/10 px-4 py-2 skeuo-leather">
          ...
          <div className="flex items-center gap-2 text-[10px] text-primary-foreground/70">
            <kbd className="thunder-badge px-1.5 py-0.5 text-[9px]">↑↓</kbd>
            <span>navigate</span>
            <kbd className="thunder-badge px-1.5 py-0.5 text-[9px]">↵</kbd>
            <span>select</span>
          </div>
