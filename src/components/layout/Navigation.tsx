...
export function Navigation({ activeSection, onNavigate }: NavigationProps) {
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-background/95 backdrop-blur-xl lg:hidden">
            <div className="flex h-full flex-col p-6">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setMobileMenuOpen(false)} className="flex h-10 w-10 items-center justify-center rounded-lg bg-secondary">
                  <X className="h-5 w-5" />
                </button>
              </div>
                ...
                {navItems.map((item, index) => {
                  ...
                  return (
                    <motion.button key={item.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1 }}
                      ...
                      className={cn("flex items-center gap-4 rounded-lg p-4 transition-all", isActive ? "bg-primary text-primary-foreground" : "bg-secondary text-foreground hover:bg-secondary/80")}>
